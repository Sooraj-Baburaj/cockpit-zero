...
import { createMemoryService, type MemoryEntry } from '../src/main/services/agent/memory-service.js';
 ...
 * scripted plan and tools but a fake memory store, a fake file port, a
 * controllable config, and a no-op delay — no electron, deterministic offline.
 */
...
describe('createTaskRunner', () => {
  it('streams the scripted plan to a review pause, then completes on approval', async () => {
    const { runner, emits, entries } = harness(configWith());
    ...
    expect(entries[0]?.text).toContain('Build a deck from the Q3 brief');
  ...
  it('blocks memory.recall when memory is off', async () => {
    const { runner, entries } = harness(configWith({ memoryEnabled: false }));
    const { taskId } = runner.start('Build a deck from the Q3 brief');
  ...
  it('stop halts the run and commits nothing', async () => {
    const { runner, entries } = harness(configWith());
    const { taskId } = runner.start('Build a deck from the Q3 brief');
