...
/**
 * `memory.recall` — keyword recall over the local memory store. Gated by
 * `ai.memoryEnabled` (via the runner's grant check AND the memory service's own
 * guard, so it returns nothing when memory is off either way). The match count is
 * a real signal, so it overrides the step narration ("matched N prior sessions").
 */
...
export const memoryRecall: Tool = {
  ...
  async run(input: unknown, ctx: ToolContext): Promise<ToolResult> {
    ...
    const hits = ctx.memory.recall(query, limit);
    return {
      ok: true,
